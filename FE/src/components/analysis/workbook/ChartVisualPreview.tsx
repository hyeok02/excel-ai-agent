    ...
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      <div className="mb-2 flex flex-wrap gap-3 text-[10px] font-bold text-slate-500">
        {series.map((item, index) => (
          <span className="inline-flex items-center gap-1.5" key={`${item.title}-${index}`}>
            <i className="size-2 rounded-full" style={{ backgroundColor: palette[index] }} />
            {item.title}
          </span>
        ))}
      </div>
        ...
        {chartType.includes('bar') ? (
            ...
            item.values.map(({ value }, valueIndex) => {
              ...
              return (
                <rect
                  ...
                  rx="2"
                  width={barWidth}
                  x={paddingX + valueIndex * groupWidth + groupWidth * 0.14 + seriesIndex * barWidth}
          ...
          series.map((item, seriesIndex) => {
            const points = item.values
              .map(({ value }, index) => {
                const x = paddingX + (index / Math.max(item.values.length - 1, 1)) * usableWidth
                return `${x},${y(value)}`
              })
              .join(' ')
            return (
