...
def assemble_report(passed, generated_count, source_limitations, overview=None):
    ...
    return ValidatedWorkbookInsightReport(
        overview=overview or (" ".join(item.fact for item in passed[:2]) if passed else (
        ...
        validation=InsightValidationSummary(
            ...
            overview_validated=bool(passed),
        ),
    )
