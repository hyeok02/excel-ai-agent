...
topic에는 title과 원본 근거에서 확인되는 대상·지표·구분 축만 짧은 명사구로 적으세요.
예를 들어 원본에 없는 회사명이나 부서명을 topic에 보충하지 말고, 주제를 확인할 수 없으면 null로 두세요.
...
category는 현재 값·구성·최솟값/최댓값 요약이면 metric, 시작·종료 시점 비교나 항목별 증감이면 change를 사용하세요.
서로 다른 시점의 관측값이 3개 이상이고 fact에 중간 시점까지 드러나 실제 흐름을 설명할 때만 trend를 사용하세요.
이상 후보는 risk를 사용하세요.
...
def build_user_prompt_from_context(
    ...
    return (
        ...
        "인사이트를 생성하세요.\n"
        "overview의 첫 문장은 반드시 '이 파일은 ... 자료입니다.' 형식으로 쓰고, 원본의 대표 대상명 또는 "
        "항목명을 그대로 하나 이상 포함해 파일이 무엇을 기록·비교·추적하는지 쉬운 말로 설명하세요. "
        "전문용어는 쉬운 한국어로 풀어 쓰고, 한 행의 회사명이나 사건을 파일 전체의 대상으로 "
        "단정하지 마세요. 파일명만 보고 의미를 추측하지 마세요.\n"
